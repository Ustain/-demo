...
<template>
  <nav style="padding: 10px">
    <div style="margin: 10px 0">
      <el-button type="primary" @click="add">预定车位</el-button>
    </div>
          ...
          <el-table-column fixed="right" label="Operations" width="150px">
            <template #default="scope">
              ...
              <el-popconfirm title="Are you sure to delete this?" @confirm="handleDelete(scope.row.id)" >
                <template #reference>
                  <el-button size="small" type="danger" >取消预定</el-button>
                </template>
              </el-popconfirm>
            </template>
          </el-table-column>
      ...
      <el-dialog v-model="dialogVisible" title="Tips" width="30%">
        <el-form :model="form" label-width="120px">
          ...
          <el-form-item label="开始时间">
            <el-input v-model="form.startTime" style="width:80%"/>
          </el-form-item>
          <el-form-item label="结束时间">
            <el-input v-model="form.endTime" style="width:80%"/>
          </el-form-item>
        </el-form>
        <template #footer>
  ...
  methods: {
    load() {
      request.get("/api/order2", {
        ...
        console.log(res);
        const userData = res.data.records.filter(item => item.tel === this.tel); // 筛选出电话号码与当前用户相同的订单数据

        this.tableData = userData;
        this.total = userData.length; // 更新total为筛选后的行数
      });
    },
    ...
    save(){
      if (this.form.id){//更新
        ...
        request.put("/api/order2",this.form).then(res =>{
          console.log(res);
          if (res.code==='0'){
            this.$message({
              type: "success",
        ...
        this.form.tel = this.tel;
        request.post("/api/order2",this.form).then(res =>{
          console.log(res);
          if (res.code==='0'){
            this.$message({
              type: "success",
    ...
    handleDetails(row) {
      // 导航到目标路由
      this.$router.push({ path: '/details2', query: { ...row } });//无脑全部传过去
    },
    handleDelete(id){
      console.log(id);
      request.delete("/api/order2/"+id).then(res => {
        if (res.code==='0'){
          this.$message({
            type: "success",
            message: "删除成功"
